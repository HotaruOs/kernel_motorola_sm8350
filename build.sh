...
TC_DIR="${PWD}/tc"
export CONFIG_FILE3="arch/arm64/configs/vendor/lineage_tundra.config"
export CONFIG_FILE2="arch/arm64/configs/vendor/lineage_moto-lahaina.config"
...
make_defconfig() {
    ...
    echo -e "${LGR}########### Merging Additional Configs ############${NC}"
    scripts/kconfig/merge_config.sh -O "${objdir}" "${objdir}/.config" ${CONFIG_FILE2} ${CONFIG_FILE3} 

    make -s ARCH=${ARCH} O=${objdir} CC=clang HOSTCC=clang olddefconfig LLVM=1 LLVM_IAS=1 -j$(nproc --all)

    END=$(date +"%s")
    echo -e "${LGR}########### Defconfig generated in  $((END - START))s ############${NC}"
}
...
completion() {
    ...
    if [[ -f ${COMPILED_IMAGE} ]]; then
        ...
        git clone -q https://github.com/HotaruOs/AnyKernel3 -b HoS $anykernel
